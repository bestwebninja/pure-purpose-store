...
function MarketplacePage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      ...
      <p className="mt-1 text-sm text-muted-foreground text-primary-foreground">Every blessing matters. Pick one to support.</p>
      {counts && <div className="mt-8"><BlessingLifecycle counts={counts} compact /></div>}
      {feed && feed.campaigns.length === 0 ? (
        <p className="mt-10 text-sm text-muted-foreground text-primary-foreground">No active blessings yet — check back soon.</p>
      ) : (
