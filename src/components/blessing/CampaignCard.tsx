...
export function CampaignCard({
  ...
  return <Link to="/campaign/$handle" params={{
    handle: campaign.handle
      ...
      <Card className="overflow-hidden border-border/60 bg-card transition-shadow hover:shadow-card">
        <div className="aspect-[4/3] overflow-hidden bg-muted">
          {campaign.image_url ? <img src={campaign.image_url} alt={campaign.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" loading="lazy" /> : <div className="h-full w-full bg-gradient-warm" />}
        </div>
        <div className="space-y-4 p-5">
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            {campaign.location && <span className="inline-flex items-center gap-1">
          ...
          {campaign.short_description && <p className="line-clamp-2 text-sm text-muted-foreground text-white">{campaign.short_description}</p>}
